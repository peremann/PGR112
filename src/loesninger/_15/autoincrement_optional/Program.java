...
public class Program {
    ...
    private void getCity() throws SQLException {
        ...
        Optional<City> city = worldService.getCity(id);
        if(city.isPresent()){
    ...
    private void addCity() throws SQLException {
        ...
        int population = Integer.parseInt(userInput.nextLine());
        int generatedId = worldService.addCity(name, countryCode, district, population);
        if(generatedId>0){
            System.out.println("City added! Id="+generatedId);
        } else{
    ...
    private void presentMainMenu() {
        System.out.println("You have 8 choices:");
